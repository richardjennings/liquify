...
expr:
  LITERAL { val := $1; $$ = MakeLiteralExpr(val) }
| IDENTIFIER { name := $1; $$ = MakeIdentExpr(name) }
| expr PROPERTY { $$ = MakePropertyExpr($1, $2) }
...
filtered:
  expr
| filtered '|' IDENTIFIER { $$ = makeFilterExpr($1, $3, nil) }
| filtered '|' KEYWORD filter_params { $$ = makeFilterExpr($1, $3, $4) }
;
